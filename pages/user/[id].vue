<template>
  <AwesomeArticle>
    ...
    <form>
      Filters:
      <span>
        <label for="change-multiple"
          >All {{ allItemsChecked ? "completed" : "pending" }}</label
        >
...
const { todos } = await useRequestUserTodoList();
...
const updateTodoStatus = (id: number, value: boolean) => {
  if (!todos.value) return;
